...
## Using test.dart (results database)

The newer `test.dart` entrypoint reads test status from the **results database** instead of deprecated status files. Use for accurate pass/fail reporting:

```bash
# Run with named configuration
./tools/sdks/dart-sdk/bin/dart tools/test.dart -n <config-name> language

# Find available configurations matching your setup
./tools/sdks/dart-sdk/bin/dart tools/test.dart --find-configurations -c dartk -r vm

# Run a local config but use status from a different bot config
./tools/sdks/dart-sdk/bin/dart tools/test.dart -n analyzer-asserts-linux -N analyzer-asserts-mac
```

Named configurations are defined in `tools/bots/test_matrix.json`. Use `--find-configurations` to discover configs matching your compiler/runtime/platform.
